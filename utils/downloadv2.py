...
def download(model: str, output: str, threads=1) -> str:
    model, branch = sanitize_model_and_branch_names(model, 'main')
    # Getting the download links from Hugging Face
    links, sha256, is_lora = get_download_links_from_huggingface(model, branch, text_only=False)
    # Getting the output folder
    output_folder = get_output_folder(model, branch, is_lora, base_folder=output)
